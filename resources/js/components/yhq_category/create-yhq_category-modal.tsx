...
export default function CreateYhqCategoryModal({ yhq_id }: { yhq_id: number }) {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        post(route('yhq_category.store'), {
            preserveScroll: true,
            onSuccess: () => { reset(); setData('yhq_id', yhq_id); clearErrors(); setOpen(false); toast.success(t('created_successfully')); },
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-md border bg-white dark:border-gray-700 dark:bg-gray-900">
                <DialogTitle className="dark:text-white">{t('yhq_category')}</DialogTitle>
                <form onSubmit={submit} className="mt-4 space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                        <input type="checkbox" id="yc-is-active" checked={data.is_active} onChange={(e) => setData('is_active', e.target.checked as true)} />
                        <Label htmlFor="yc-is-active" className="dark:text-gray-200">{t('active_status')}</Label>
                    </div>
                    <DialogFooter>
                        <DialogClose asChild>
                            <Button variant="secondary" onClick={() => { reset(); setData('yhq_id', yhq_id); clearErrors(); }} className="dark:bg-gray-800 dark:text-gray-200">{t('cancel')}</Button>
                        </DialogClose>
