...
import { ServerError, YhqCategory } from '@/types';

export default function UpdateYhqCategoryModal({ category }: { category: YhqCategory }) {
    const { t } = useTranslation();
    const [open, setOpen] = useState(false);

    const { data, setData, put, processing, errors } = useForm({
        yhq_id: category.yhq_id,
        name: category.name || '',
        is_active: category.is_active,
    });

    useEffect(() => {
        setData({ yhq_id: category.yhq_id, name: category.name, is_active: category.is_active });
    }, [category]);

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        put(route('yhq_category.update', category.id), {
            preserveScroll: true,
                ...
                <DialogContent className="max-w-md border bg-white dark:border-gray-700 dark:bg-gray-900">
                    ...
                    <form onSubmit={submit} className="mt-4 space-y-4">
                        ...
                        <div className="flex items-center gap-2">
                            <input type="checkbox" id={`yc-active-${category.id}`} checked={data.is_active} onChange={(e) => setData('is_active', e.target.checked)} />
                            <Label htmlFor={`yc-active-${category.id}`} className="dark:text-gray-200">{t('active_status')}</Label>
                        </div>
                        <DialogFooter>
                            <DialogClose asChild>
