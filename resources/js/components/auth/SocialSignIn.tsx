...
const SocialSignIn = () => {
    ...
    useEffect(() => {
        // Clear previous widget to prevent duplicates on re-renders
        const container = document.getElementById('telegram-container');
        if (container) container.innerHTML = '';

        // Inject Telegram widget dynamically
        const script = document.createElement('script');
        script.src = 'https://telegram.org/js/telegram-widget.js?22'; // Updated version
        script.async = true;
        script.setAttribute('data-telegram-login', 'PlusAvto.Uz');
        script.setAttribute('data-size', 'large');
        script.setAttribute('data-userpic', 'false');
        script.setAttribute('data-request-access', 'write');
        script.setAttribute('data-auth-url', 'https://panel.plusavto-uz.uz/auth/telegram/callback');
        script.setAttribute('data-radius', '10');
        script.setAttribute('data-lang', 'en');

        // Dark mode support for Telegram Widget
        // Checks if 'dark' class exists on documentElement (standard for Tailwind dark mode)
    ...
    // const buttonStyle = `
    //     flex w-full items-center justify-center gap-2.5 rounded-lg border p-3.5
    ...
    return (
        <div className="w-full space-y-4">
            ...
            {/*    /!* Google Button *!/*/}
            {/*    <button onClick={() => signIn('google')} className={buttonStyle}>*/}
            {/*        <svg width="20" height="20" viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg">*/}
            {/*            <path*/}
            {/*                d="M22.5001 11.2438C22.5134 10.4876 22.4338 9.73256 22.2629 8.995H11.7246V13.0771H17.9105C17.7933 13.7929 17.5296 14.478 17.1352 15.0914C16.7409 15.7047 16.224 16.2335 15.6158 16.646L18.9264 19.3124L19.1571 19.335C21.2772 17.4161 22.4997 14.5926 22.4997 11.2438"*/}
            {/*                fill="#4285F4"*/}
            {/*            />*/}
            {/*            <path*/}
            ...
            {/*    /!* GitHub Button *!/*/}
            {/*    <button onClick={() => signIn('github')} className={buttonStyle}>*/}
            ...
            {/* Telegram Section */}
            <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-gray-300 p-4 dark:border-slate-700">
                <p className="mb-3 text-xs font-semibold tracking-wider text-gray-500 uppercase dark:text-gray-400">{t('auth.or_sign_in_with')}</p>
                <div id="telegram-container" className="flex min-h-[40px] items-center justify-center"></div>
            </div>
        </div>
    );
};
